...
def find_hotels_high(message: types.Message, destination: str, error=False):
    """
    ...
    try:
        ...
        button_yes = types.InlineKeyboardButton(text='Да!', callback_data=f'y|high|{hotels_count}|{destination}')
        button_no = types.InlineKeyboardButton(text='Нет =(', callback_data=f'n|high|{hotels_count}|{destination}')
        ...
        find_hotels_high(message, destination, error=True)
