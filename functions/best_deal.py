...
def find_hotels_best(message: types.Message, destination: str, error=False):
    """
    ...
    try:
        ...
        button_yes = types.InlineKeyboardButton(text='Да!', callback_data=f'y|best|{hotels_count}|{destination}')
        button_no = types.InlineKeyboardButton(text='Нет =(', callback_data=f'n|best|{hotels_count}|{destination}')
        ...
        find_hotels_best(message, destination, error=True)
